...
def all_triangle_lines(rows, columns):
    ...
    # Rows - highest point
    shifts = rows - triangle[len(triangle) // 2]
...
def generate_lines(rows, columns):
    ...
    # 4 -- horizontals
    all_horizontal = all_horizontal_lines(rows, columns)

    # 8 -- triangle
    all_triangle = all_triangle_lines(rows, columns)
    ...
    for y in range(rows - 1):
        norm_dips.append(shift_points(dip, (0, y)))
        flip_dips.append(shift_points(flip_dip, (0, y)))

    all_dip = [norm_dips, flip_dips]
    # --------
    # 20 -- saw & flipped
